...
final class SchedulePendingListViewModel: ObservableObject {
    ...
    @Published var hasNotDoneWorkdays: [WorkdayEntity] = [] {
        didSet {
            let result = sortHasNotdoneWorkdaysByDate()
            self.sortedHasNotDoneWorkdays = result
        }
    }
...
private extension SchedulePendingListViewModel {
    ...
    func getSortedHasNotDoneWorkdays() {
        ...
        print(result.count)
        print(hasNotdoneWorkdaysBeforeToday.count)
        hasNotDoneWorkdays = hasNotdoneWorkdaysBeforeToday
    }
    
    // 미확정 일정을 날짜 별로 분류합니다.
    func sortHasNotdoneWorkdaysByDate() -> [(Date, [WorkdayEntity])] {
        let dateArray = hasNotDoneWorkdays.map { $0.date }
